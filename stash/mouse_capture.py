...
class MouseSensor():
    ...
    def read_endpoint(self):
        ...
        return self.device.read(endpoint.bEndpointAddress,
                                endpoint.wMaxPacketSize)

    def read_register(self, register, length):
        return self.device.ctrl_transfer(bmRequestType=0xC0,
                                         bRequest=0x01,
                                         wValue=0x0000,
                                         wIndex=register,
    ...
    def write_register(self, register, value, length):
        return self.device.ctrl_transfer(bmRequestType=0x40,
                                         bRequest=0x01,
                                         wValue=value,
                                         wIndex=register,
                                         data_or_wLength=length)
...
canvas = tkinter.Canvas(root,
                        width=mouse.sensor_size * scale,
                        height=mouse.sensor_size * scale)
canvas.pack()
...
def update_frame():
    ...
    image = ImageTk.PhotoImage(
        image=mouse.grab_frame().resize((mouse.sensor_size * scale,
                                         mouse.sensor_size * scale)))
