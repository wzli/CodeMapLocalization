...
bool DmlsGen::is_word_visited(uint32_t word) const {
  uint32_t last_visit_ids[] = {
    _word_visit_table[word], 
...
bool DmlsGen::parse_start_word(uint32_t& start_word, const std::vector<bool>& sequence) {
  start_word = 0;
...
void DmlsGen::propagate_explored_rates(uint32_t node) {
    for(double explored_rate_increment = 1.0; node !=  Node::Index::NONEXISTANT; node = _nodes[node].parent) {
...
double DmlsGen::generate_dmls(std::vector<bool> &sequence, uint8_t word_length, uint32_t iterations) {
  ...
  while(_nodes[0].explored_rate < 1.0 && --iterations) {
      ...
      ++_visit_id;
      while (next_word_explored_rate[0] < 1.0 || next_word_explored_rate[1] < 1.0) {
        if(sequence.size() < _word_len) {
            next_word[0] = word;
        ...
        for(uint8_t i = 0; i < 2; ++i) {
            switch (_nodes[node].child[i]) {
              ...
              default:
                next_word_explored_rate[i] =_nodes[node].child[i] == Node::Index::NONEXISTANT ? 1.0 :_nodes[_nodes[node].child[i]].explored_rate;
            }
        }
        uint8_t next_bit = next_word_explored_rate[0] >  next_word_explored_rate[1];
