...
void DmlsGen::generate_dmls(std::vector<bool> &sequence, uint8_t word_length, uint32_t iterations,  std::function<void(std::vector<bool> &sequence)> new_record_callback) {
    ...
    while(--iterations) {
        ...
        while(true) {
            ...
            for(uint8_t i = 0; i < 4; ++i) {
                next_word_weights[i] = is_word_visited(next_words[i]) ? 0.0 : 1.0/(_word_visit_counts[next_words[i]] + 1);
                weight_sum +=  next_word_weights[i];
            }
            if(weight_sum == 0.0) {
                break;
            }
            _next_word_selector.param(std::discrete_distribution<uint16_t>::param_type(next_word_weights.begin(), next_word_weights.end()));
