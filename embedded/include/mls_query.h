...
uint32_t mlsq_code_from_position(const uint32_t* sequence, uint32_t code_length, uint16_t position);
...
static inline uint32_t mlsq_code_from_position_indexed(uint32_t code_length, uint16_t position) {
