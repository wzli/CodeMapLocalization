...
void print_image_matrix(ImageMatrix src) {
    for (int32_t row = 0; row < src.n_rows; ++row) {
        for (int32_t col = 0; col < src.n_cols; ++col) {
...
void print_axis_code(AxisCode axis_code) {
    int32_t leading_zeros = first_set_bit(reverse_bits(axis_code.mask, 32));
...
uint32_t test_diff_bits(uint32_t a, uint32_t b) {
    return MIN(sum_bits(a ^ b), sum_bits(~a ^ b));
}

uint32_t sizeof_img_type() {
